...
import com.example.jogodavelhagabriel.databinding.ActivityMainBinding
import kotlin.random.Random

class ModoMaquinaActivity : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        buttonSelecionado.setBackgroundResource(R.drawable.lakers)
        ...
        while (i < 9) {
            ...
            if (tabuleiro[rX][rY] != "LAKERS" && tabuleiro[rX][rY] != "CELTICS") {
                break
            }
        ...
        tabuleiro[rX][rY]="CELTICS"

        val posicao = rX*3 + rY

        when(posicao){
            0 -> {
                binding.buttonZero.setBackgroundResource(R.drawable.celtics)
                binding.buttonZero.isEnabled = false
            }
            1 -> {
                binding.buttonUm.setBackgroundResource(R.drawable.celtics)
                binding.buttonUm.isEnabled = false
            }
            2 -> {
                binding.buttonDois.setBackgroundResource(R.drawable.celtics)
                binding.buttonDois.isEnabled = false
            }
            3 -> {
                binding.buttonTres.setBackgroundResource(R.drawable.celtics)
                binding.buttonTres.isEnabled = false
            }
            4 -> {
                binding.buttonQuatro.setBackgroundResource(R.drawable.celtics)
                binding.buttonQuatro.isEnabled = false
            }
            5 -> {
                binding.buttonCinco.setBackgroundResource(R.drawable.celtics)
                binding.buttonCinco.isEnabled = false
            }
            6 -> {
                binding.buttonSeis.setBackgroundResource(R.drawable.celtics)
                binding.buttonSeis.isEnabled = false
            }
            7 -> {
                binding.buttonSete.setBackgroundResource(R.drawable.celtics)
                binding.buttonSete.isEnabled = false
            }
            8 -> {
                binding.buttonOito.setBackgroundResource(R.drawable.celtics)
    ...
    fun verificaVencedor(tabuleiro: Array<Array<String>>): String? {
        ...
        for (linha in tabuleiro) {
            for (valor in linha) {
                if(valor.equals("LAKERS")||valor.equals("CELTICS")){
                    empate++
                }
            }
        }
